...
public class TenkokuModuleEditor : Editor {
	...
	public override void OnInspectorGUI() {
        ...
        if (script.showConfig){
        	EditorGUI.DrawPreviewTexture(new Rect(rt.x+margin,rt.y+435,387,34),divRevTex);
			GUILayout.Space(10.0f);
		    ...
		    GUI.Label (new Rect (rt.x+margin+10, rt.y+265, 190, 15), new GUIContent("Link Clouds to System"));
			script.cloudLinkToTime = EditorGUI.Toggle(new Rect(rt.x+margin+165, rt.y+265, 30, 18), "", script.cloudLinkToTime);
			ToolTip(rt.x+margin+140, rt.y+265,"Cloud speed and offset will be multiplied by the 'Advance Time' settings.");

		    GUI.Label (new Rect (rt.x+margin+10, rt.y+285, 190, 15), new GUIContent("Use Legacy Clouds"));
			script.useLegacyClouds = EditorGUI.Toggle(new Rect(rt.x+margin+165, rt.y+285, 30, 18), "", script.useLegacyClouds);
			ToolTip(rt.x+margin+120, rt.y+285,"Switch cloud rendering to the Legacy Cloud system.  Legacy Clouds use less resources at the cost of visual fidelity.");
				...
				GUI.contentColor = colorDisabled;
    			GUI.backgroundColor = colorDisabled;
				GUI.Label (new Rect (rt.x+margin+210, rt.y+285, 190, 15), new GUIContent("(Unity 5.3.4+ setting only)"));
				GUI.contentColor = colorEnabled;
    			GUI.backgroundColor = colorEnabled;
			#endif
			...
			if (script.useLegacyClouds){
         	...
         	GUI.Label (new Rect (rt.x+margin+10, rt.y+305, 180, 15), new GUIContent("Cloud Quality"));
        	script.cloudQuality = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+305, setWidth, 18), "", script.cloudQuality,0.25f,2.0f);
			ToolTip(rt.x+margin+88, rt.y+305,"Strikes a balance between performance and quality.  The higher the quality the fewer noise artifacts will be visible in the clouds.  Higher quality will have a marked improvement in this respect but comes at a noticeable performance hit.  Default is 0.6.");

        	GUI.Label (new Rect (rt.x+margin+10, rt.y+325, 180, 15), new GUIContent("Cloud Frequency"));
        	script.cloudFreq = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+325, setWidth, 18), "", script.cloudFreq,0.1f,3.0f);
			ToolTip(rt.x+margin+107, rt.y+325,"Changes the overall dispercement of clouds from few large clous to frequent small clouds.  Default is 1.");

        	GUI.Label (new Rect (rt.x+margin+10, rt.y+345, 180, 15), new GUIContent("Cloud Detail"));
        	script.cloudDetail = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+345, setWidth, 18), "", script.cloudDetail,0.1f,3.0f);
			ToolTip(rt.x+margin+84, rt.y+345,"Changes the frequency of edge detail, making clouds look straight or fluffy.  Default is 1.");

        	GUI.contentColor = colorEnabled;
    		GUI.backgroundColor = colorEnabled;
	        	
	        GUI.Label (new Rect (rt.x+margin+10, rt.y+365, 180, 15), new GUIContent("Precipitation Quality"));
	        script.precipQuality = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+365, setWidth, 18), "", script.precipQuality,0.01f,2.0f);
			ToolTip(rt.x+margin+127, rt.y+365,"Controls number of rain and snow particles emitted.  Higher quality emits more particles at the cost of more system resources.  Lower this setting for better performance.  Default is 1.");





			script.enableAutoAdvance = EditorGUI.Toggle(new Rect(rt.x+margin+10, rt.y+395, 20, 18),"", script.enableAutoAdvance);
			...
			EditorGUI.LabelField(new Rect(rt.x+margin+30, rt.y+395, 340, 18),"AUTO-ADVANCE SYSTEM TIMER");
	        script.systemTime = EditorGUI.FloatField(new Rect(rt.x+margin+260, rt.y+395, 120, 18),"",script.systemTime);
			
        	//GUI.contentColor = colorDisabled;
        	//GUI.backgroundColor = colorDisabled;
        	GUI.contentColor = new Color(0.54f,0.55f,0.55f,1.0f);
        	EditorGUI.LabelField(new Rect(rt.x+margin+30, rt.y+415, 340, 18),"the 'systemTime' variable is automatically advanced by");
        	EditorGUI.LabelField(new Rect(rt.x+margin+30, rt.y+427, 340, 18),"default.  This variable can be shared across a network to");
			EditorGUI.LabelField(new Rect(rt.x+margin+30, rt.y+439, 340, 18),"sync cloud positions between client and server computers.");
			GUI.contentColor = colorEnabled;
        	GUI.backgroundColor = colorEnabled;


        	GUILayout.Space(435.0f);
        }
        GUILayout.Space(10.0f);
        ...
        if (script.showColors){
			...
			GUI.Label (new Rect (rt.x+margin+10, rt.y+50, 100, 15), new GUIContent ("Overall Tint"));
			script.colorOverlay = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+50, setWidth, 15), script.colorOverlay);

			GUI.Label (new Rect (rt.x+margin+10, rt.y+70, 100, 15), new GUIContent ("Sky Tint"));
			script.colorSky = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+70, setWidth, 15), script.colorSky);

			GUI.Label (new Rect (rt.x+margin+10, rt.y+90, 100, 15), new GUIContent ("Sun Scattering"));
			script.colorSkyboxMie = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+90, setWidth, 15), script.colorSkyboxMie);

			GUI.Label (new Rect (rt.x+margin+10, rt.y+110, 100, 15), new GUIContent ("Skybox Ground"));
			script.colorSkyboxGround = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+110, setWidth, 15), script.colorSkyboxGround);

			GUI.Label (new Rect (rt.x+margin+10, rt.y+130, 100, 15), new GUIContent ("Fog Tint"));
			script.colorFog = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+130, setWidth, 15), script.colorFog);

			GUI.Label (new Rect (rt.x+margin+10, rt.y+150, 100, 15), new GUIContent ("Lightning Tint"));
			script.lightningColor = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+150, setWidth, 15), script.lightningColor);
			...
			GUI.Label (new Rect (rt.x+margin+10, rt.y+170, 100, 15), new GUIContent ("Ambient Tint"));
			script.colorAmbient = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+170, setWidth, 15), script.colorAmbient);
			ToolTip(rt.x+margin+83, rt.y+170,"Works in conjunction the Ambient Probe: Color or Ambient Probe: Gradient settings only.");
			...
			GUI.Label (new Rect (rt.x+margin+10, rt.y+190, 150, 15), new GUIContent ("Day Ambient Amount"));
        	script.ambientDayAmt = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+190, setWidth, 18), "", script.ambientDayAmt,0.0f,5.0f);
			ToolTip(rt.x+margin+135, rt.y+190,"Works in conjunction the Ambient Probe: Skybox setting only.  To adjust ambient brightness when set to color or gradient mode, please adjust the ambient color gradient or texture accordingly.");
			
			GUI.Label (new Rect (rt.x+margin+10, rt.y+210, 150, 15), new GUIContent ("Night Ambient Amount"));
        	script.ambientNightAmt = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+210, setWidth, 18), "", script.ambientNightAmt,0.0f,25.0f);
			ToolTip(rt.x+margin+140, rt.y+210,"Works in conjunction the Ambient Probe: Skybox setting only.  To adjust ambient brightness when set to color or gradient mode, please adjust the ambient color gradient or texture accordingly.");

			GUI.contentColor = colorEnabled;
			GUI.backgroundColor = colorEnabled;


			GUI.Label (new Rect (rt.x+margin+10, rt.y+230, 150, 15), new GUIContent ("Set Ambient Colors"));
			script.useAmbient = EditorGUI.Toggle(new Rect(rt.x+margin+165, rt.y+230, 60, 15), "", script.useAmbient);
			ToolTip(rt.x+margin+122, rt.y+230,"Turns off Tenkoku Ambient contribution, allowing you to control ambient colors from elsewhere in your project.");




        	GUI.Label (new Rect (rt.x+margin+10, rt.y+260, 190, 15), new GUIContent("COLOR GRADIENT SETTINGS"));

        	GUI.Label (new Rect (rt.x+margin+10, rt.y+280, 190, 15), new GUIContent("Gradient Source"));
        	string[] colorTypeOptions = new string[]{"Texture","Custom Gradient"};
        	script.colorTypeIndex = EditorGUI.Popup(new Rect(rt.x+margin+165, rt.y+280, setWidth, 18),"",script.colorTypeIndex, colorTypeOptions);

        	if (script.colorTypeIndex == 0){
         		GUI.Label (new Rect (rt.x+margin+20, rt.y+305, 100, 15), new GUIContent ("Color Texture"));
        		script.colorRamp = EditorGUI.ObjectField(new Rect(rt.x+margin+165, rt.y+305, setWidth, 35), script.colorRamp, typeof(Texture2D), true) as Texture2D;
        		
        		GUILayout.Space(320.0f);
        	}
        	...
        	if (script.colorTypeIndex == 1){
                ...
                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+305, 120, 18),"Cloud Color");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+305, setWidth, 18),cloudGradColor,new GUIContent(""),false);

                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+325, 120, 18),"Cloud Ambient");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+325, setWidth, 18),cloudAmbGradient,new GUIContent(""),false);

                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+345, 120, 18),"Sun Color");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+345, setWidth, 18),sunGradient,new GUIContent(""),false);

                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+365, 120, 18),"Horizon Color");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+365, setWidth, 18),horizGradient,new GUIContent(""),false);

                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+385, 120, 18),"Sky Color");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+385, setWidth, 18),skyGradient,new GUIContent(""),false);

                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+405, 120, 18),"Ambient Color");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+405, setWidth, 18),ambColorGradient,new GUIContent(""),false);

                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+425, 120, 18),"Moon Color");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+425, setWidth, 18),moonGradient,new GUIContent(""),false);

                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+445, 120, 18),"Ambient Daylight");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+445, setWidth, 18),ambDayGradient,new GUIContent(""),false);

                EditorGUI.LabelField(new Rect(rt.x+margin+20, rt.y+465, 120, 18),"Ambient Lighting");
				EditorGUI.PropertyField(new Rect(rt.x+margin+165, rt.y+465, setWidth, 18),ambLightGradient,new GUIContent(""),false);


				objGradColor.ApplyModifiedProperties();


        		GUILayout.Space(465.0f);
        	}
        ...
        if (script.showConfigAtmos){
        	EditorGUI.DrawPreviewTexture(new Rect(rt.x+margin,rt.y+290,387,34),divRevTex);
			GUILayout.Space(10.0f);
        	...
        	GUI.Label (new Rect (rt.x+margin+10, rt.y+145, 180, 15), new GUIContent("Start Fog"));
        	script.fogAtmosphere = EditorGUI.FloatField(new Rect(rt.x+margin+165, rt.y+145, setWidth, 18), "", script.fogAtmosphere);

        	GUI.Label (new Rect (rt.x+margin+10, rt.y+165, 180, 15), new GUIContent("Fog Distance"));
        	script.fogDistance = EditorGUI.FloatField(new Rect(rt.x+margin+165, rt.y+165, setWidth, 18), "", script.fogDistance);

        	GUI.Label (new Rect (rt.x+margin+10, rt.y+185, 180, 15), new GUIContent("Fade Distance"));
        	script.fadeDistance = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+185, setWidth, 18), "", script.fadeDistance,0.0f,1.0f);

        	GUI.Label (new Rect (rt.x+margin+10, rt.y+205, 180, 15), new GUIContent("Fog Density"));
        	script.fogDensity = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+205, setWidth, 18), "", script.fogDensity,0.0f,1.0f);

        	GUI.Label (new Rect (rt.x+margin+10, rt.y+225, 180, 15), new GUIContent("Fog Dispersion"));
     		script.fogDispersion = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+225, setWidth, 18), "", script.fogDispersion,0f,10f);
			...
			//LIGHT RAYS
			GUI.Label (new Rect (rt.x+margin+10, rt.y+255, 150, 15), new GUIContent ("Calculate Light Rays"));
			script.useSunRays = EditorGUI.Toggle(new Rect(rt.x+margin+165, rt.y+255, 60, 15), "", script.useSunRays);
			
			if (!script.useSunRays){
				GUI.contentColor = colorDisabled;
				GUI.backgroundColor = colorDisabled;
			}
			GUI.Label (new Rect (rt.x+margin+10, rt.y+275, 150, 15), new GUIContent ("Sun Ray Intensity"));
        	script.sunRayIntensity = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+275, setWidth, 18), "", script.sunRayIntensity,0.0f,1.0f);

        	GUI.Label (new Rect (rt.x+margin+10, rt.y+295, 150, 15), new GUIContent ("Sun Ray Length"));
        	script.sunRayLength = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+295, setWidth, 18), "", script.sunRayLength,0.0f,1.0f);
     		
     		GUI.contentColor = colorEnabled;
			GUI.backgroundColor = colorEnabled;

        	GUILayout.Space(290.0f );
        }
        GUILayout.Space(10.0f);
        ...
        if (script.showConfigWeather){
			...
			if (script.weatherTypeIndex == 0){ //custom
				EditorGUI.DrawPreviewTexture(new Rect(rt.x+margin,rt.y+420,387,34),divRevTex);
				...
				GUI.Label (new Rect (rt.x+margin+10, rt.y+335, 180, 15), new GUIContent("Temperature"));
				...
				GUI.Label (new Rect (rt.x+margin+10, rt.y+355, 180, 15), new GUIContent("Rainbow"));
	        	script.weather_rainbow = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+355, setWidth, 18), "", script.weather_rainbow,0.0f,1.0f);



				GUI.Label (new Rect (rt.x+margin+10, rt.y+385, 180, 15), new GUIContent("Lightning Amount"));
	        	script.weather_lightning = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+385, setWidth, 18), "", script.weather_lightning,0.0f,1.0f);

				GUI.Label (new Rect (rt.x+margin+10, rt.y+405, 180, 15), new GUIContent("Lightning Direction"));
	        	script.weather_lightningDir = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+405, setWidth, 18), "", script.weather_lightningDir,0.0f,359.0f);

				GUI.Label (new Rect (rt.x+margin+10, rt.y+425, 180, 15), new GUIContent("Lightning Range"));
	        	script.weather_lightningRange = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+425, setWidth, 18), "", script.weather_lightningRange,0.0f,180.0f);


				GUILayout.Space(420.0f);
			}


			if (script.weatherTypeIndex == 1){ //AUTO Simple
				EditorGUI.DrawPreviewTexture(new Rect(rt.x+margin,rt.y+80,387,34),divRevTex);
				...
				GUI.Label (new Rect (rt.x+margin+10, rt.y+45, 180, 15), new GUIContent ("Next Random Pattern"));
				script.weather_forceUpdate = EditorGUI.Toggle(new Rect(rt.x+margin+165, rt.y+45, 60, 15), "", script.weather_forceUpdate);  
			
	        	GUI.Label (new Rect (rt.x+margin+10, rt.y+65, 180, 15), new GUIContent("Weather Pattern Lifetime"));
	        	script.weather_autoForecastTime = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+65, setWidth, 18), "", script.weather_autoForecastTime,0.01f,120.0f);
	        	
	        	GUI.Label (new Rect (rt.x+margin+10, rt.y+85, 180, 15), new GUIContent("Weather Transition Speed"));
	        	script.weather_TransitionTime = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+85, setWidth, 18), "", script.weather_TransitionTime,0.001f,60.0f);

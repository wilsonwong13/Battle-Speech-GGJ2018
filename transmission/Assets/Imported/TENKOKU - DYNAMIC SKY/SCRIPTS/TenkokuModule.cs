	...
	//if (Application.isPlaying){
		...
		if (useAmbient){
			...
			if (ambientTypeIndex == 0){
				...
				UnityEngine.RenderSettings.ambientIntensity = Mathf.Lerp(ambientNightAmt, ambientDayAmt, lightObjectWorld.intensity);
		...
		if (!auroraIsVisible || auroraLatitude > Mathf.Abs(setLatitude)){
			...
			if (useCameraCam != null){
				//csSize = ((useCameraCam.farClipPlane / 20f) * 1.75f);
				auroraScale.x = csSize + 0.5f;
				auroraScale.y = csSize + 0.5f;
				auroraScale.z = csSize + 0.5f;
			}
	...
	void TimeUpdate(){
		...
		if (useAutoTime && !autoTimeSync && Application.isPlaying){
			...
			if (useTimeCompression < 3600.0f){
				...
				setTimeSpan = (_deltaTime * (useTimeCompression/60.0f));
...
//if (position == 5) returnColor = Color.black;
	...
	public void Tenkoku_DecodeData( string dataString ){
		...
		weather_rainbow = float.Parse(data[25]);
		weather_lightning = float.Parse(data[26]);
		weather_lightningDir = float.Parse(data[27]);
		weather_lightningRange = float.Parse(data[28]);

		//decode random marker
		randSeed = int.Parse(data[29]);
		TenRandom = new Tenkoku.Core.Random( randSeed );

		//current cloud coordinates
		currCoords.x = float.Parse(data[30]);
		currCoords.y = float.Parse(data[31]);
